...
> 사용자가 "옵시디언", "노트", "메모", "Inbox" 등을 언급하면 아래 규칙에 따라 동작.
...
- **파일 접근**: Filesystem MCP (`/Users/shinjeayoun` 전체)

### 볼트 폴더 구조
| 폴더 | 용도 |
|------|------|
| `01-Memory/` | 사용자 정보, 비즈니스 구조, 도구, 우선순위 |
| `03-Actions/` | 실행 계획, 액션 아이템 |
| `04-AI-Research/` | AI/기술 리서치 결과 저장 |
| `05-Content/` | 콘텐츠 가이드, 글쓰기 규칙 |
| `06-Templates/` | 템플릿 |
| `📥 Inbox/` | 핸드폰 메모 (정리 대상) |
| `📋 정리됨/` | 정리 완료된 노트 |
| `📅 일정/` | 일정 노트 (구글 캘린더 연동) |
...
2. 마크다운 요약 (출처 URL 포함)
3. REST API PUT → `04-AI-Research/YYYY-MM-DD-주제.md` 저장
4. 대화창에 요약 보고

**"Inbox 정리" / "옵시디언 정리" / "메모 정리"**
1. REST API로 `📥 Inbox/` 내 모든 파일 읽기
2. 카테고리 분류: 일정→📅, 할일→✅, 일반→📋
3. 각 폴더에 정리된 노트 저장 (`#정리완료` 태그)
4. 일정 발견 시 구글 캘린더 등록 제안
5. 원본 Inbox 메모는 삭제하지 않음
6. "N개 정리 완료 (일정 X, 할일 Y, 일반 Z)" 형태로 보고
...
**"옵시디언에 저장해줘" (대화 결과물)**
1. 내용에 따라 적절한 폴더 자동 선택
2. REST API PUT으로 저장
3. 파일명: `YYYY-MM-DD-주제.md`

### API 사용법
```bash
# 파일 읽기
curl -s -H "Authorization: Bearer $API_KEY" http://127.0.0.1:27123/vault/{URL인코딩경로}
...
# 검색
curl -s -H "Authorization: Bearer $API_KEY" "http://127.0.0.1:27123/search/simple/?query=검색어"
```

### 규칙
- 허용 폴더 외 파일 수정/삭제 금지
- Inbox 원본은 보존 (사용자가 직접 삭제)
- 파일명: 한국어 OK, 날짜 `YYYY-MM-DD` 포맷
- 정리된 노트에 `#정리완료` 태그 추가
