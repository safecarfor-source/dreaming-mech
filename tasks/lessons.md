...
## 커뮤니케이션 규칙 (2026-03-21 추가)

### #상황판 = 작업 기록
1. **대장님이 `#상황판`으로 시작하면**: 현재 상황/이슈를 기록하고, 해당 문제를 분석 → 해결 방안 제시 → 즉시 구현
2. **스크린샷 첨부 시**: 스크린샷에서 문제점을 시각적으로 파악하고 코드와 대조
3. **간결한 지시 해석**: 대장님의 짧은 메모(예: "영상 로딩 렉", "연상 최적화")를 구체적 기술 과제로 변환하여 실행

---

*마지막 업데이트: 2026-03-21 (#상황판 규칙 추가)*
