...
> 새 기능을 추가할 때 이 지도를 먼저 보고, 어느 모듈에 "문"을 다는지 확인한다.
> 벽 너머 모듈은 건드리지 않는다. 벽을 옮겨야 하면 이 문서를 먼저 수정한다.

```
┌─────────────────────────────────────────────────────────────────┐
│                        EC2 서버 (1대)                            │
│  ┌───────────────────────────────────────────────────────────┐  │
│  │  통합 프론트엔드 (Next.js :3000)                          │  │
│  │                                                           │  │
│  │  [플랫폼]              [인센티브 — Next.js App Router]     │  │
│  │  /                     /incentive/login                    │  │
│  │  /community            /incentive/team (팀 인센티브)       │  │
│  │  /admin                /incentive/manager (김권중)         │  │
│  │  /mechanic/*           /incentive/director (이정석)        │  │
│  │  /region/*             /incentive/settlement (월말결산)    │  │
│  │  /shop/*               /incentive/insight (인사이트)       │  │
│  │                        /incentive/inventory (시재관리)     │  │
│  │                        /incentive/admin (관리자 설정)      │  │
│  └──────────────────────────────┬────────────────────────────┘  │
│             │ API                            │ API               │
│  ┌──────────▼───────────────────────────────▼────────────────┐  │
│  │                  백엔드 (NestJS :3001)                      │  │
...
│  │  │                  │  ║  │                            │  │  │
│  │  │ /auth/*          │  ║  │ /incentive/auth/*          │  │  │
│  │  │ /mechanics       │  ║  │ /incentive/upload          │  │  │
│  │  │ /service-inquiry │  ║  │ /incentive/calc            │  │  │
│  │  │ /community       │  ║  │ /incentive/gd/*  ← GD조회  │  │  │
...
│  │  │                                                       │  │  │
│  │  │  [플랫폼 테이블]     ║  [인센티브 테이블]              │  │  │
│  │  │  Mechanic            ║  IncentiveUser                 │  │  │
│  │  │  Owner               ║  IncentiveData                 │  │  │
│  │  │  Customer            ║  IncentiveUpload               │  │  │
│  │  │  ServiceInquiry      ║  IncentiveConfig               │  │  │
│  │  │  Post/Comment        ║  ProductCodeMapping            │  │  │
│  │  │  Admin               ║  ManagerIncentiveData          │  │  │
│  │  │                      ║  DirectorIncentiveData         │  │  │
│  │  │  ═══════════════════════════════════════════════════  │  │  │
│  │  │  [공유 테이블 — 벽 양쪽에서 읽음]                     │  │  │
│  │  │  GdVehicle (slot A/B)     GdSlotConfig               │  │  │
│  │  │  GdRepair (slot A/B)      GdSyncLog                  │  │  │
│  │  │  GdProduct (slot A/B)     CashFlow                   │  │  │
...
│  ┌─────────────────────────────────────────────────────────────┐  │
│  │  극동 동기화 (Python)                                        │  │
│  │  gd_sync_server.py → incoming/TOTAL.GDB → A/B시소 → DB     │  │
│  │  백업: hourly×24 + daily×7 + monthly 30일                   │  │
│  │  C격리: slots/C/ 5개 FIFO                                   │  │
│  └─────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────┘
          ↑ SCP/SFTP
...
### 벽(║) 규칙
- **플랫폼 모듈**과 **인센티브 모듈**은 서로의 테이블을 직접 수정하지 않는다
- **공유 테이블(GD*)**: 양쪽에서 읽기(SELECT)만 허용. 쓰기는 gd_sync_server.py만
- **erp 모듈**: 유일하게 벽을 넘는 모듈. GD 테이블을 읽지만, 수정은 하지 않음
- **스키마 변경**: 공유 테이블 변경 시 양쪽 모듈 모두 빌드 검증 필수

### 확장 포인트 (새 문 다는 곳)
| 새 기능 유형 | 어디에 달면 되는가 | 주의 |
|-------------|-------------------|------|
| 새 인센티브 탭 | 인센티브 프론트 + /incentive/* API | 플랫폼 모듈 무관 |
| 새 플랫폼 페이지 | 플랫폼 프론트 + 플랫폼 API | 인센티브 모듈 무관 |
| 새 GD 분석 기능 | incentive/gd 서비스에 메서드 추가 | slot 필터 필수 |
| 새 DB 테이블 | 해당 모듈 영역에 추가 | 공유 영역은 양쪽 검증 |
| 카카오 알림톡 | 새 모듈 생성 (notification) | 기존 모듈 수정 최소화 |

---

## 💰 인센티브 시스템 설계 (2026-04-01 확정)

### 계정 구조
| 계정 | 아이디 | 역할 | 접근 가능 탭 |
|------|--------|------|------------|
| 관리자 | admin | admin | 전체 |
| 김권중 | kwonjoong | manager | 김권중, 팀 인센티브, 월말결산, 인사이트, 시재관리 |
| 이정석 | jeongsuk | director | 이정석, 팀 인센티브, 월말결산, 인사이트, 시재관리 |

### 인센티브 계산 구조

#### 서비스 품목별 요율 (팀 인센티브 = 1인당 금액)
| 품목 | 요율(%) |
|------|--------|
| 브레이크오일 | 2.8 |
| 라이닝 | 1.4 |
...
#### 이정석 부장 (매출 인센티브)
- 기본 연봉: 42,960,000원 (월 3,580,000원)
- **전년 대비 성장률 5구간 요율:**
...
#### 김권중 매니저 (타이어+서비스 인센티브)
- 기본 연봉: 39,600,000원 (월 3,300,000원)
- 계산: **(타이어+얼라인먼트) × 0.3%** + **서비스인센(1인당) × 1.5배**
- TPMS 매출은 타이어에 포함됨 (정상)

#### 기사 (서비스 인센티브)
- 서비스인센(1인당) × 인원수(7명)
- 최소수량 미달 시 50% 감액

### 전년 매출 기준 (2025년 하드코딩, team/page.tsx)
```
1월: 141,000,000  2월: 119,870,650  3월: 141,360,490  4월: 152,108,207
5월: 145,727,900  6월: 125,293,560  7월: 161,988,450  8월: 158,435,400
9월: 167,255,850  10월: 183,964,700  11월: 175,339,060  12월: 212,235,800
```

### 인센티브 기술 스택
- 프론트: Next.js App Router + Zustand persist (incentive-auth)
- 백엔드: NestJS /incentive/* 모듈 (auth, team, calc, director, manager, gd)
- 극동 데이터: gd_sync_server.py → A/B 시소 → DB → calc-engine 자동계산
- 인증: 별도 JWT (IncentiveUser 테이블, 플랫폼 Auth와 독립)
...
| 2026-03-24 | A/B 시소 + C 격리 DB 시스템 구현 (설계+코드+서버배포) | ✅ 완료 |
...
| 2026-04-01 | 인센티브 4개 개선: 4월 자동전환, 5개월 이력, 이정석 요율 변경, hydration fix | ✅ 완료·배포 |
| 2026-04-01 | 인센티브 설계 문서화: 엑셀 대조 검증, TPMS 포함 확인, BRAIN.md 구조도 업데이트 | ✅ 완료 |
